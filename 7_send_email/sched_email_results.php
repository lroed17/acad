...
if (!empty($_REQUEST["useremail"])) {
    $message = "Class Schedule Search Results\r";
    $message .= "--------------------------------\r";

    while ($currentrow = $results->fetch_assoc()) {
        $message .= "Class " . $currentrow["class"] .
            " (Type " .
            $currentrow["type"] .
            "),  " .
    ...
    $test = mail($_REQUEST["useremail"], "Your Class Schedule Results", $message);
    echo "<hr><em>(results emailed to " . $_REQUEST["useremail"] . ")</em><hr><br>";
    $results->data_seek(0);

    echo "Your mail was sent? Answer: ". $test. "<br><br>";
}
